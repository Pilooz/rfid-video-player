...
var msgWaitingMedia = { uri: CONFIG.app.mediaPath + "/messages/" + CONFIG.app.messages.msgWaitingMediaFile, loop: "on", autoplay: "on", controls: "off", status: "waiting", tag: "", reader: "" },
    msgMediaNotFoundMedia = { uri: CONFIG.app.mediaPath + "/messages/" + CONFIG.app.messages.msgMediaNotFoundMediaFile, loop: "off", autoplay: "on", controls: "off", status: "mediaNotFound", tag: "", reader: "" },
    msgNoTagAssocMedia = { uri: CONFIG.app.mediaPath + "/messages/" + CONFIG.app.messages.msgNoTagAssocMediaFile, loop: "off", autoplay: "on", controls: "off", status: "noTagAssociation", tag: "", reader: "" },
    msgSearchingMedia = { uri: CONFIG.app.mediaPath + "/messages/" + CONFIG.app.messages.msgSearchingMediaFile, loop: "off", autoplay: "on", controls: "off", status: "searching", tag: "", reader: "" };
...
module.exports = {
	...
	mediaNotFoundMedia: function(tag, reader, file) { 
		msgMediaNotFoundMedia.tag = tag;
		msgMediaNotFoundMedia.reader = reader;
		msgMediaNotFoundMedia.filename = file;
		return msgMediaNotFoundMedia; 
	},

	noTagAssocMedia: function(tag, reader) { 
		msgNoTagAssocMedia.tag = tag;
		msgNoTagAssocMedia.reader = reader;
	...
	//
	chooseMedia: function(tag, reader, rootPath) {
		// Video data sctructure for the choosen media
		var mediaFile = { uri: "", loop: "", autoplay: "", controls: "", status: "", tag: "", reader: "" },	
			medias = [];
		...
		if ( medias.length == 0 ) {
			return module.exports.noTagAssocMedia(tag, reader);
		} else {
			mediaFile = { uri: module.exports.chooseRandomly(medias), loop: "off", autoplay: "on", controls: "on", status: "content", tag: tag, reader: reader };
			// Verifying that file exists
			if (!fs.existsSync(rootPath +  mediaFile.uri)) { 
				return module.exports.mediaNotFoundMedia(tag, reader, mediaFile.uri);
			}
