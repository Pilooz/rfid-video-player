...
module.exports = {
	...
	sort_unique: function(arr) {
  	...
  	buildKeywordList: function(rfidTag) {
	  ...
	  db_keywords.keywordslist.forEach(function(k){
		...
		return this.sort_unique(keyList);
  },

  buildMediaList: function(rfidTag, path){
	  // Builded media list from keywordList
	  var mediaCollection = [];
	  var index = 0;
	  // First Building keyword List 
	  var keys = this.buildKeywordList(rfidTag);
	  console.log(keys);  
	  ...
	  var col = this.sort_unique(mediaCollection);
	  console.log(col); 
	  return col;
	}
}